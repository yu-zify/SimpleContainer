...
if [ "$pam" == "1" ]; then
	 $WHOME/busybox tar -xvf $WHOME/core -C $WHOME
	echo 1
elif [ "$pam" == "2" ]; then
  if test -f /sdcard/Download/debian_xfce.tar.gz ; then
    mv /sdcard/Download/debian_xfce.tar.gz $WHOME
  fi
	 $WHOME/busybox tar -zxvf $WHOME/debian_xfce.tar.gz -C $WHOME
	echo 2
elif [ "$pam" == "3" ]; then
   $WHOME/busybox tar -zxvf $WHOME/extra/extra.tar.gz -C $WHOME/test
fi
echo ok
